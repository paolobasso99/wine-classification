...
wine
```
All the columns are continuos a part from the quality one.
Free sulfure dioxide and Total sulfure dioxid are continuous covariates even thought by the values it seems they are discrete. 
They are continuous since they are measures (mg/L) [Conventional Measurements of Sulfur Dioxide (SO2) in Red Wine Overestimate SO2 Antimicrobial Activity ~ https://www.extension.iastate.edu/wine/wp-content/uploads/2019/04/conventional_so2_overestimates_in_red_wine_ajev_2018.pdf].
Furthermore, with more attention we can find continuous value at rows 433 and 439.
...
Plotting the histogram distributions for each column of the dataframe wine.
...
```
What we notice is the presence of long tails for most of the columns, this suggests
the presence of outliers.
A part from the alcohol, sulphates, total.sulfur.dioxide distributions, if we remove
the tails data are symmetrically distributed.
Quality data are highly concentrated in the range of values (5- 7): since the 
quality is our target value, we may encounter some issues during the predictions
as the data are unbalanced.
