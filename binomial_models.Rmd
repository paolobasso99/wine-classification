...
colnames(wine)
```
total_sulfur.dioxide, density, chlorides, residual.sugar, free.sulfur.dioxide, sulphates, citric.acid
...
error/ subN
```
# Comparison between Gaussian prior and Lasso prior

```{r}
load("chains/binomial_model.dat")
gaussian_m = output

load("chains/binomial_lasso.dat")
lasso_m = output

p <- ggplot_compare_all_d(lasso_m, gaussian_m, 11)
ggsave("pictures/binomial_comparison.png", plot=p)
```
